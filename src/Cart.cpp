...
void
Cart::loadRom(const std::string& filename)
throw(Exception)
{
    ...
    if(f.is_open())  {
        ...
        for(i = 0x0104; i <= 0x0133; i++)  {
            if(data[i] != nintendo_logo[i-0x0104])  {
                throw( Exception(Exception::WRONG_NINTENDO_LOGO) );
            }
        }
        ...
        if((checksum & 0xff) != 0)
            throw( Exception(Exception::BAD_HEADER_CHECKSUM) );
        else
            is_loaded = true;
    } else  {
        throw( Exception(Exception::CANNOT_OPEN_ROM_FILE) );
    }
}
